...
async function setNewScore(score: number, name: string) {
...
function sortTuples(a: [number, string], b: [number, string]) {
    if (a[0] < b[0]) return 1;
    if (a[0] == b[0]) return 0;
    if (a[0] > b[0]) return -1;
}
...
function getScores() {
    let resultTable: [number, string][] = [];
    let newScore: number = Number(localStorage.getItem("currentScore"));
    ...
    function viewLeaderboard(): void {
        ...
        let newFirstElement: HTMLElement;
        let newSecondElement: HTMLElement;
    ...
    function viewFormToAddNewScore() {
        ...
        let usernameForm = <HTMLInputElement>document.getElementById("usernameForm");
